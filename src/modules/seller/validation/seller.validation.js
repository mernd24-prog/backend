...
const sellerChargeSettingsBodySchema = Joi.object({
  ...
  delivery: Joi.object({
    ...
    serviceabilityMode: Joi.string().valid("all_pincodes", "allowlist", "blocklist", "regions", "disabled"),
    allowPincodes: Joi.alternatives().try(
      Joi.array().items(Joi.string().trim()),
      Joi.string().allow("", null),
    ),
    ...
    productRules: Joi.array().items(Joi.object().unknown(true)),
    orderRules: Joi.array().items(Joi.object().unknown(true)),
    regionRules: Joi.array().items(Joi.object().unknown(true)),
    estimatedDaysMin: Joi.number().integer().min(0).allow(null),
    estimatedDaysMax: Joi.number().integer().min(0).allow(null),
    shippingPartner: Joi.string().max(160).allow("", null),
    shippingMethod: Joi.string().max(80).allow("", null),
    handlingCharge: Joi.number().min(0),
    notes: Joi.string().max(1000).allow("", null),
  }),
  metadata: Joi.object(),
}).min(1);
