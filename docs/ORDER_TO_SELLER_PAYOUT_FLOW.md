...
flowchart LR
    ...
    D --> E[Order receipt and marketplace tax documents]
...
Payment capture or authorization commits inventory, commits deal sales, creates
the order receipt and marketplace tax-document bundle, and clears purchased cart
items. Payment failure releases inventory, wallet, and deal reservations and
...
| --- | --- | --- |
| `order_customer` | platform -> buyer | automatic non-tax order receipt after payment authorization/capture |
| `seller_customer` | seller -> buyer | automatic marketplace bundle generation |
| `platform_commission` | platform -> seller | automatic when commission is non-zero |
| `platform_customer_fee` | platform -> buyer | automatic when customer platform fee GST is non-zero |

The marketplace bundle is grouped by `(sellerId, organizationId)`. The order
receipt is its non-tax parent, each seller invoice covers that seller's items and
delivery charge, and each product-linked commission invoice covers seller-borne
marketplace fees plus configured fee GST.

The order receipt records no TCS and posts no product GST ledger entries. GST
TCS is calculated from each seller's GST-exclusive taxable supply snapshot and
withheld in settlement only.

### 9.2 Tax ledger

Legal seller and marketplace service invoices post positive `tax_collected`
ledger entries. The order receipt posts none. Credit-note creation posts
`tax_reversed` entries for CGST, SGST, and IGST.
...
- The aggregate order receipt is reused by order ID and document type.
...
Failed QC is a reviewed dispute state rather than an automatic refund denial.
The seller must attach item notes and evidence images for every `missing` or
`rejected` decision. The buyer can dispute that evidence during the recorded
48-hour window. An administrator can request more evidence, uphold the failure,
override it for the full eligible item refund, or approve item quantities
partially. The affected item payout remains held throughout review. When a
failure is upheld, the hold is released only after the rejected product is
delivered back to the customer (unless return shipping is explicitly waived).
When QC is overridden, the normal item-level refund, credit-note, commission,
tax and payout-adjustment flow continues.
